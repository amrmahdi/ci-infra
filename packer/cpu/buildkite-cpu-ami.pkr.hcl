...
variable "buildkite_commit" {
  type        = string
  default     = ""
  description = "Commit SHA for cache key parity with CI builds"
}
...
build {
  ...
  provisioner "shell" {
    environment_vars = [
      ...
      "BUILDKITE_COMMIT=${var.buildkite_commit}",
      "VLLM_USE_PRECOMPILED=${var.vllm_use_precompiled}",
      "VLLM_MERGE_BASE_COMMIT=${var.vllm_merge_base_commit}"
    ]
    inline = [
      "sudo -u buildkite-agent -i ECR_TOKEN=\"$ECR_TOKEN\" BUILDKITE_COMMIT=\"$BUILDKITE_COMMIT\" VLLM_USE_PRECOMPILED=\"$VLLM_USE_PRECOMPILED\" VLLM_MERGE_BASE_COMMIT=\"$VLLM_MERGE_BASE_COMMIT\" bash /tmp/scripts/warm-cache.sh"
    ]
  }
